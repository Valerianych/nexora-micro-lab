import {readProgress, writeProgress, restoreGame, serializeGame, signalSnapshot} from './progress.js';
...
const caseCatalog = [
  ...
  {
    id: '003', title: 'Пульт забыл команды',
    problem: 'Контроллер теряет последовательность световых команд между запусками.',
    board: 'Arduino Uno', topics: ['массивы', 'индексы'], image: artSource('c1-resistor-found', 'thumb'), playable: false,
  },
  {
    id: '004', title: 'Одинаковые команды',
    problem: 'В программе слишком много повторов — нужно собрать их в понятные инструменты.',
    board: 'Arduino Uno', topics: ['функции', 'параметры'], image: artSource('c2-program-repair', 'thumb'), playable: false,
  },
  {
    id: '005', title: 'Серверная перегревается',
    problem: 'Вентилятор включается поздно: датчик уже показывает опасную температуру.',
    board: 'ESP32', topics: ['датчики', 'условия'], image: artSource('c1-roof-arrival', 'thumb'), playable: false,
  },
  {
    id: '006', title: 'Аварийный протокол',
    problem: 'Нужно объединить цепь, код и датчики в одну систему, которой доверяет комиссия.',
    board: 'Arduino + ESP32', topics: ['сборка', 'логика', 'отладка'], image: artSource('scene-briefing', 'thumb'), playable: false,
  },
];
...
function renderArchive() {
  ...
  caseCatalog.forEach((item, index) => {
    ...
    const action = playable
      ...
      : solved
        ? '<button class="case-report-action" type="button">Прочитать отчёт ✓</button>'
        : unlocked
...
function beginCase(id) {
  const item = caseInfo(id);
  if (!item || !state.unlockedCases.has(id) || !item.playable) return;
  dismissCaseReveal();
  $('archive-modal')?.close();
  state.activeCase = id;
  state.started = true;
  resetCaseProgress({ resetScore: id === '001' });
...
function showCaseReveal(currentId, nextId) {
  ...
  reveal.innerHTML = `<div class="case-reveal-scrim"></div><section class="case-reveal-stage"><div class="case-paper-stack">${casePaperMarkup(current, 'case-paper-done', 'ЗАКРЫТО')}${casePaperMarkup(next, 'case-paper-next', 'НОВОЕ')}</div><div class="case-reveal-copy"><span class="eyebrow">ДЕЛО ${current.id} ЗАКРЫТО / АРХИВ ПОПОЛНЕН</span><h2>Появилось новое расследование.</h2><p>${canStart ? `Пока ты возвращала маяку голос, в серверной сбился аварийный ритм. Дело ${next.id} уже ждёт следующего решения.` : `Следующая карточка уже появилась в архиве. Сюжет и отдельный верстак для неё откроются следующим обновлением.`}</p><div class="case-reveal-note"><b>Новый фокус</b><span>${next.topics.join(' · ')}</span></div><div class="case-reveal-actions"><button class="accent case-reveal-primary" type="button">${canStart ? `Открыть дело ${next.id}` : 'Открыть архив дел'} <span>→</span></button><button class="case-reveal-later" type="button">Остаться в архиве</button></div></div></section>`;
...
function renderStats() {
  ...
  setText('inventory-count', `${state.inventory.length} / ${state.activeCase === '002' ? 2 : 4}`);
...
function updateMap() {
  ...
  document.querySelectorAll('.room').forEach((button) => {
    ...
    button.classList.toggle('complete', state.activeCase === '002' ? state.case2Reached > ({briefing:0,workshop:2,server:4,roof:5}[room]) || (room === 'roof' && state.final) : state.completed.has({ workshop: 0, server: 1, roof: 2 }[room]));
...
function resetCase() {
  dismissCaseReveal();
  state.activeCase = '001';
  state.started = false;
  state.skills = new Set();
  state.completedCases = new Set();
  state.unlockedCases = new Set(['001']);
  resetCaseProgress({ resetScore: true });
...
function showMissionTransition(mission, name) {
  ...
  const close = () => {
    ...
    window.setTimeout(() => { transition.remove(); renderRoom(); }, 260);
  };
...
window.addEventListener('nexora:mission-complete', (event) => {
  ...
  const allowed = state.activeCase === '001' ? mission >= 0 && mission < 3 : mission === 3 && state.case2Stage === 4;
